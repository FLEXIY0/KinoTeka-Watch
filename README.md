...
Bun это ~90 МБ бинарника. Если машина слабая, а Node.js уже стоит, качать его
незачем — клиент одинаково работает на обоих:

```bash
curl -fsSL https://raw.githubusercontent.com/FLEXIY0/KinoTeka-Watch/main/cli/install.sh | KTW_RUNTIME=node sh
```

`KTW_RUNTIME` принимает `bun` (по умолчанию), `node` и `auto` — последний берёт
то, что уже есть в системе, и ничего не скачивает.

> **Требования**: [Bun](https://bun.sh) **или** [Node.js](https://nodejs.org) 18+
> (любой ставится скриптом автоматически, без root), [mpv](https://mpv.io).
>
> Node строго 18 или новее: в нём появился глобальный `fetch`, на котором
> держится вся работа с балансерами. Если в системе Node постарше, установщик
> сам положит свежий в `~/.local/lib/node`.
...
ktw --doctor                 # Диагностика: рантайм, сеть, балансеры, окружение
